...
const Dashboard = () => {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      students: s.status === 'fulfilled' ? (s.value?.data?.data?.length || 0) : 0,
      teachers: t.status === 'fulfilled' ? (t.value?.data?.data?.length || 0) : 0,
      marks: m.status === 'fulfilled' ? (m.value?.data?.data?.length || 0) : 0,
    }));
  }, []);
  ...
  return (
    <div className="animate-fade-in">
      <div className="welcome-section" style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div className="welcome-text">
          <h1>Admin Dashboard 👋</h1>
          <p>Manage your school — students, teachers, marks, and credentials.</p>
        </div>
        <Btn id="download-excel-btn" onClick={() => alert('Downloading Excel...')}>📥 Download Excel</Btn>
      </div>
...
const StudentsList = () => {
  ...
  const handleAddStudent = async () => {
    ...
    try {
      await api.post('resource/School Student', payload);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} style={{ padding: '9px 12px 9px 32px', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: '0.9rem', outline: 'none' }} />
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {grades.map(grade => (
            <div key={grade} className="glass-panel" style={{ padding: '0', overflow: 'hidden' }}>
              ...
              {expanded[grade] && (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.8fr 1.5fr 0.8fr 130px', padding: '8px 20px', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-secondary)', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-secondary)' }}>
                  ...
                  {byGrade[grade].map((s, i) => (
                    <div key={s.name} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.8fr 1.5fr 0.8fr 130px', padding: '12px 20px', alignItems: 'center', borderBottom: i < byGrade[grade].length - 1 ? '1px solid var(--border-color)' : 'none', transition: 'background 0.15s' }}
                      ...
                      <div style={{ textAlign: 'center', display: 'flex', gap: '8px', justifyContent: 'center' }}>
                        <Btn small variant="success" onClick={() => alert('Edit soon')}>✏️ Edit</Btn>
...
const TeachersList = () => {
  ...
  const handleAddTeacher = async () => {
    ...
    try {
      await api.post('resource/School Teacher', payload);
      ...
      {showAdd && (
        <Modal title="Add New Teacher" onClose={() => setShowAdd(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <Field label="Email"><Input value={addForm.email} onChange={v => setAddForm({ ...addForm, email: v })} /></Field>
            <Field label="Primary Subject (e.g. Mathematics)"><Input value={addForm.subject} onChange={v => setAddForm({ ...addForm, subject: v })} /></Field>
            {addMsg && <div style={{ padding: '10px', borderRadius: '8px', background: 'rgba(108,92,231,0.1)', fontSize: '0.9rem' }}>{addMsg}</div>}
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <Btn variant="danger" onClick={() => setShowAdd(false)}>Cancel</Btn>
      ...
      {modal === 'edit' && (
        <Modal title="Edit Marks Entry" onClose={() => setModal(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <Field label="Subject * (e.g. Mathematics)"><Input value={form.subject} onChange={v => setForm({ ...form, subject: v })} /></Field>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <Field label="Marks Obtained * (e.g. 0)"><Input type="number" value={form.marks_obtained} onChange={v => setForm({ ...form, marks_obtained: v })} /></Field>
              <Field label="Max Marks (e.g. 100)"><Input type="number" value={form.max_marks} onChange={v => setForm({ ...form, max_marks: v })} /></Field>
            </div>
            <Field label="Exam Date"><Input type="date" value={form.exam_date} onChange={v => setForm({ ...form, exam_date: v })} /></Field>
            <Field label="Remarks (Optional)"><Input value={form.remarks} onChange={v => setForm({ ...form, remarks: v })} /></Field>
            {msg && <div style={{ padding: '10px', borderRadius: '8px', background: 'rgba(108,92,231,0.1)', fontSize: '0.9rem' }}>{msg}</div>}
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
